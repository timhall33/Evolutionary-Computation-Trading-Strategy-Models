...
def ep_mutation(individual, mutation_step_size):
...
def evolution_strategy(data):
    ...
    toolbox.register("mutate", ep_mutation)
    ...
    for generation in range(num_generations):
        ...
        # Apply boundaries to mutation step size
        ...
        scores = {i: 0 for i in range(len(combined_population))}

        for i, ind in enumerate(combined_population):
            opponents = random.sample(combined_population, 5)
            for opponent in opponents:
                if i != combined_population.index(opponent):
                    if ind.fitness.values > opponent.fitness.values:
                        scores[i] += 1

        # Select top solutions based on the number of wins
        top_indices = sorted(scores.keys(), key=lambda x: scores[x], reverse=True)[:len(population)]
        top_individuals = [combined_population[i] for i in top_indices]
    ...
    output_file_json = 'saved_individuals_11.json'
